import { Crumb, HashHistoryManager, StateNavigator, State, StateInfo } from 'navigation';
...
// Configuration
const config: StateInfo<'people' | 'person'>[] = [
  {
    key: 'people',
    route: ['people/{page}', 'people/{page}/sort/{sort}'],
    defaults: { page: 1 },
    help: 'people.htm',
  },
  {
...
person.urlEncode = function urlEncode(
  state: State,
  key: string,
  val: string,
  queryString: boolean
...
person.urlDecode = function urlDecode(
  state: State,
  key: string,
  val: string,
  queryString: boolean
