...
class TabBar extends React.Component<any, any> {
  ...
  render() {
    ...
    var tabBarItems = React.Children.toArray(children).filter(
      (child) => !!child
    );
    var titleOnly = !tabBarItems.find(
      ({ props }: any) => props.title && props.image
    );
    var { fontFamily, fontWeight, fontStyle, fontSize } =
    ...
    var tabLayout = (Platform.OS === "android" || !primary) && (
      <TabView
        ...
        tabs={tabBarItems.map(({ props: { title, testID } }: any) => ({
          title,
          testID,
        }))}
