/**
 * Navigation React Mobile v3.3.0
...
var NavigationReactMobile = (function (exports,navigation,React,navigationReact) {
    ...
    var MobileHistoryManager = /** @class */ (function (_super) {
        ...
        MobileHistoryManager.prototype.addHistory = function (url, replace, stateContext) {
            var title = typeof document !== 'undefined' && document.title;
            if (!!stateContext && !stateContext.history) {
                var oldUrl = stateContext.oldUrl, crumbs = stateContext.crumbs;
                var start = !oldUrl ? 0 : oldUrl.split('crumb=').length;
                for (var i = start; i < crumbs.length; i++) {
                    var _a = crumbs[i], url_1 = _a.url, state = _a.state;
                    _super.prototype.addHistory.call(this, url_1, i === 0);
                    if (typeof document !== 'undefined' && state.title)
                        document.title = state.title;
                }
            }
            _super.prototype.addHistory.call(this, url, replace);
            if (title)
    ...
    var NavigationMotion = /** @class */ (function (_super) {
        ...
        NavigationMotion.getDerivedStateFromProps = function (_a, _b) {
            ...
            if (stateNavigator === prevStateNavigator)
                return null;
            var _c = stateNavigator.stateContext, state = _c.state, crumbs = _c.crumbs, nextCrumb = _c.nextCrumb;
    ...
    var useNavigating = function (handler) {
        var navigationEvent = React.useContext(navigationReact.NavigationContext);
        React.useEffect(function () {
            var stateNavigator = navigationEvent.stateNavigator;
            var beforeNavigateHandler = function (state, data, url, history, currentContext) {
                var crumb = url.split('crumb=').length - 1;
                var _a = stateNavigator.stateContext, sceneCrumbs = _a.crumbs, sceneState = _a.state;
    ...
    var useNavigated = function (handler) {
        var navigationEvent = React.useContext(navigationReact.NavigationContext);
        React.useEffect(function () {
            handler();
        }, [navigationEvent]);
    ...
    var useUnloading = function (handler) {
        var navigationEvent = React.useContext(navigationReact.NavigationContext);
        React.useEffect(function () {
            var stateNavigator = navigationEvent.stateNavigator;
            var beforeNavigateHandler = function (state, data, url, history, currentContext) {
                ...
                if (!(sceneCrumbs.length === crumb && sceneState === state)
                    ...
                    var crumbs = stateNavigator.parseLink(url).crumbs;
    ...
    var useUnloaded = function (handler) {
        var navigationEvent = React.useContext(navigationReact.NavigationContext);
        React.useEffect(function () {
            var cancelled = false;
            var stateNavigator = navigationEvent.stateNavigator;
            var navigateHandler = function (_oldState, state, data, _asyncData, stateContext) {
    ...
    exports.useNavigating = useNavigating;
    exports.useNavigated = useNavigated;
    exports.useUnloading = useUnloading;
    exports.useUnloaded = useUnloaded;
